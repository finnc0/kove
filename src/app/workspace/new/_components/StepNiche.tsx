...
export function StepNiche({ value, onChange, onContinue }: Props) {
    ...
    <div className="space-y-0 w-full">
      ...
      <div className="flex justify-center mt-10">
        <button
          ...
          disabled={!value.trim()}
          className={cn(
            "px-8 py-2.5 text-sm font-medium rounded-lg bg-white text-zinc-900 transition-all",
            !value.trim() ? "opacity-40 cursor-not-allowed" : "hover:bg-zinc-100 cursor-pointer"
          )}
        >
          Continue →
        </button>
